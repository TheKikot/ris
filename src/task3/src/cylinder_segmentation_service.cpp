...
void 
cloud_cb (void (&)(task3::GetLocation::Request&, task3::GetLocation::Response&))
{
  // All the objects needed
  
  // getting the 
	// const pcl::PCLPointCloud2ConstPtr& cloud_blob
	const pcl::PCLPointCloud2ConstPtr& cloud_blob = ros::topic::waitForMessage("input", nh);
  ...
  seg.setInputNormals (cloud_normals2);
	...
	std::cerr << "Looking for cylinders with a radius between 0.1 and 0.15 m" << std::endl;
...
main (int argc, char** argv)
{
  ...
  // Host a service for cylinder detection
	cyl_service = nh.advertiseService("detect_cylinder", cloud_cb);
