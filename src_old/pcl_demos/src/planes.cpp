...
void handler (const pcl::PCLPointCloud2ConstPtr& cloud_blob)
{
	std::cout << "Called handler" << std::endl;
}
...
main (int argc, char** argv)
{
  ...
  ros::Subscriber sub = nh.subscribe ("/camera/depth_registered/image_raw", 10, handler);//cloud_cb);

  // Create a ROS publisher for the output point cloud
  pub = nh.advertise<pcl::PCLPointCloud2> ("planes", 1);

	std::cout << "Waiting for planes" << std::endl;
  // Spin
  ros::spin ();
}
